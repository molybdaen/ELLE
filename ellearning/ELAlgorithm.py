...
def func():
    return MNIST_DATA[0], MNIST_DATA[1], 10
...
if __name__ == "__main__":
    ...
    def test_compound(hidden_size):
        ...
        for i in range(0, hidden_size, 10):
            ...
            strt = time.time()
            logs = ae.train(train_data[0], elastic=False, epochs=None, mini_batch_size=20, stop_err_delta=0.01, supervisedDataCallback=func)
            end = time.time()
            print "Total Training Time: %.3f" % (end-strt)
